...
grouped_df.loc[grouped_df['id'].isin(['左重心前移', '右重心前移']),'id'] = '重心前移'
grouped_df.loc[grouped_df['id'].isin(['头部倾斜', '头部水平']),'id'] = '头倾斜'
...
grouped_df.loc[grouped_df['id']=='重心前移','id'] = '重心{}移'.format(oren.values[0])
...
# 调整人体图片大小，保持透明背景
resieze_height = row_height * 14.5
body_image_ratio = resieze_height/ body_image.height if resieze_height < body_image.height else body_image.height / resieze_height
...
# 画布设置
start_x = body_image_x - body_image.width
start_y = body_image_y - 50
column_spacing = body_image.width * 2 + 100  # 两列之间的间隔
row_spacing = 40  # 每个方框之间的垂直间隔
...
for idx, (index, row) in enumerate(df.iterrows()):
    ...
    # 绘制抗锯齿折线
    cv2.line(cv2_image, start_point_int, (int(bend_x), int(bend_y)), (0, 0, 0), 2, cv2.LINE_AA)
    cv2.line(cv2_image, (int(bend_x), int(bend_y)), tuple(map(int, end_point)), (0, 0, 0), 2, cv2.LINE_AA)
...
start_y = box_y + int(row_height*2.5)
...
# 更新起始Y坐标
current_y = comprehensive_title_y + (comprehensive_title_bbox[3] - comprehensive_title_bbox[1]) + 30  # 标题下方间距 30px

# 定义每列的宽度
column_width = (canvas_width - 2 * text_x) // 2
column_spacing = 30  # 两列之间的间隔

# 定义每列的起始 X 坐标
column1_x = text_x
column2_x = text_x + column_width + column_spacing
...
# 加载字体
font_text = ImageFont.truetype("demo/SIMYOU.TTF", size=25)
font_bold = ImageFont.truetype("demo/heiti.ttf", size=25)  # 假设没有单独的加粗字体路径

# 纠正建议内容
suggestions = [
    ("头部倾斜（右）和头部水平偏移（右）：", [
        "建议： 加强倾斜反侧肌肉，拉伸倾斜侧肌肉。",
        "训练动作：斜方肌拉伸（针对倾斜侧）、下巴回缩练习、侧颈部拉伸"
    ]),
    ("颈椎前倾：", [
        "建议： 加强深颈屈肌，改善姿势。",
        "训练动作：下巴回缩、胸椎伸展练习"
    ...
    ("双肩水平不平（右肩高）：", [
        "建议： 加强较低肩膀侧肌肉，拉伸较高肩膀侧肌肉。",
        "训练动作：肩胛骨后缩和下压练习（如划船）、上斜方肌拉伸"
    ]),
    ("骨盆前移：", [
        "建议： 加强后链肌群（臀部和腘绳肌），拉伸髋屈肌。",
        "训练动作：臀桥、髋屈肌拉伸"
    ]),
    ("骨盆轻微前倾：", [
        "建议： 加强核心和腘绳肌，拉伸下背部和髋屈肌。",
        "训练动作：平板支撑、腘绳肌拉伸、骨盆前后倾练习"
    ]),
    ("膝关节水平差异（左膝偏高）：", [
        "建议： 平衡膝部周围的肌肉力量。",
        "训练动作：台阶练习、单腿平衡训练"
    ...
    ("左脚踝不稳，足内翻（严重）：", [
        "建议： 加强足弓和踝关节稳定性，改善本体感觉。",
        "训练动作：足弓强化练习（如毛巾抓握）、踝关节强化练习（如提踵）、平衡训练（如单腿站立）"
    ])
]

# 绘制纠正建议的每一部分
current_x = column1_x
for i, (title, details) in enumerate(suggestions):
    # 检查是否需要切换到第二列
    if current_x == column1_x and current_y + 100 > canvas_height:
        current_x = column2_x
        current_y = column_y

    # 绘制小标题（如 "头部倾斜（右）和头部水平偏移（右）：")，使用加粗字体
    draw.text((current_x, current_y), title, font=font_bold, fill="black")
    title_bbox = draw.textbbox((0, 0), title, font=font_bold)
    current_y += title_bbox[3] - title_bbox[1] + 5  # 小标题与下一行的间距

    for detail in details:
        # 绘制每个建议内容，缩进显示
        draw.text((current_x + 20, current_y), detail, font=font_text, fill="black")
        detail_bbox = draw.textbbox((0, 0), detail, font=font_text)
        current_y += detail_bbox[3] - detail_bbox[1] + 3  # 内容与下一行的间距

    current_y += 10  # 每项内容间的间距
# 保存最终图像
canvas.save("demo/reports/my_posture_evaluation_report.png", format="PNG")
